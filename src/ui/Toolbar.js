...
export class Toolbar {
  constructor(viewer, options = {}) {
    ...
    const ICONS = {
      ...
      fullscreen: '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>',
      minimize: '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="4 14 10 14 10 20"/><polyline points="20 10 14 10 14 4"/><line x1="14" y1="10" x2="21" y2="3"/><line x1="3" y1="21" x2="10" y2="14"/></svg>',
    };

    // Store icons for later use (fullscreen toggle)
    this.ICONS = ICONS;
    ...
    const defaultButtons = [
      ...
      { id: 'fullscreen', group: 'view', label: 'Fullscreen', icon: ICONS.fullscreen, action: () => this._toggleFullscreen() },
  ...
  _updateFullscreenIcon() {
    const fullscreenBtn = this.container.querySelector('.potree-toolbar-button[data-id="fullscreen"]');
    if (fullscreenBtn) {
      const iconSpan = fullscreenBtn.querySelector('.potree-button-icon');
      if (document.fullscreenElement) {
        iconSpan.innerHTML = this.ICONS.minimize;
        fullscreenBtn.title = 'Exit Fullscreen';
      } else {
        iconSpan.innerHTML = this.ICONS.fullscreen;
  ...
  _createButton(config) {
    ...
    // Add data-id attribute for all buttons
  ...
  _attachEventListeners() {
    ...
    document.addEventListener('msfullscreenchange', () => this._updateFullscreenIcon());
  }
